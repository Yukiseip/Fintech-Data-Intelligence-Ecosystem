...
echo "========================================"
echo " Step 0: Truncate staging tables"
echo "========================================"
PGPASSWORD=fintech_secret_2026 psql -h postgres -U fintech_user -d fintech_dw -c "
TRUNCATE TABLE staging.fact_transactions_staging;
TRUNCATE TABLE staging.dim_users_staging;
TRUNCATE TABLE staging.dim_merchants_staging;
"

echo "========================================"
echo " Step 1: Re-running Silver for all 29 days"
echo "========================================"
...
echo "========================================"
echo " Step 2: Gold Modeling for all 29 days"
echo "========================================"
for DAYS_AGO in $(seq 29 -1 1); do
    TARGET_DATE=$(date -u -d "${DAYS_AGO} days ago" "+%Y-%m-%d")
    echo "  [Gold] ${TARGET_DATE}..."
    spark-submit ${SPARK_CONF} /opt/spark/jobs/gold_modeling.py --date "${TARGET_DATE}" --bucket "${BUCKET}" 2>&1 | grep -E "\[INFO\] __main__|ERROR|Exception" | tail -3
done
...
echo "========================================"
PGPASSWORD=fintech_secret_2026 psql -h postgres -U fintech_user -d fintech_dw -c "
SELECT 'fact_transactions' AS table_name, count(*) AS rows FROM gold.fact_transactions
UNION ALL
